...
# set up treatment parameters, 150 conditions
...
grid_dim = 50
densities = [5]
minspace = [2.e-13, 2e-11, 2e-12, 2e-10]
initial_carbon = [4e-10, 4e-7, 4e-4]
...
parameter_dict = {'growth_rate': growth_rates, 'production_cost': production_costs, 
                  ...
                  'replicate': replicates, 'space_widths': space_width_starts, 'densities': densities, 'threshold': minspace, 'carbon': initial_carbon}
...
for i in expanded_grid.index.values:
    ...
    threshold = expanded_grid.at[i, 'threshold']
    carbon = expanded_grid.at[i, 'carbon']
    ...
    p.set_param('minSpaceBiomass', threshold)
    ...
    p.set_param("allowCellOverlap", False)

    ## Make the layout
    l = c.layout(susceptible_models + producer_models + resistant_models)
    l.grid = grid_size
    l.set_specific_metabolite("carbon_e", carbon)
    ...
    biomass_total["minimum_biomass"] = threshold
    biomass_total["carbon_conc"] = carbon

    total_biomass_data = pd.concat([total_biomass_data, biomass_total])

    full_biomass = sim.biomass
    biomass = full_biomass[full_biomass["cycle"] == max_cycles]
    biomass["growth_rate"] = growth_rate
    biomass["spatial_seed"] = spatial_seed
    biomass["minimum_biomass"] = threshold
    biomass["carbon_conc"] = carbon

    biomass_data = pd.concat([biomass_data, biomass])

    print(i + 1, "of", max(expanded_grid.index.values) + 1)

total_biomass_data.to_csv("/Users/abisesi/Desktop/PhD/Projects/Toxins-Growth-Rate/toxin-simulations/for_reviewers/total_biomass_no-cell-overlap.csv")
biomass_data.to_csv("/Users/abisesi/Desktop/PhD/Projects/Toxins-Growth-Rate/toxin-simulations/for_reviewers/individual_biomass_no-cell-overlap.csv")
