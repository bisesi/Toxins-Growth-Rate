...
prod_res_growth_rates = [0.125, 1]
sus_growth_rates = [0.25, 0.5, 1, 1.5, 2]
...
parameter_dict = {'prod_growth_rate': prod_res_growth_rates, 'sus_growth_rate': sus_growth_rates, 'production_cost': production_costs, 
...
base = f"/Users/abisesi/Desktop/PhD/Projects/Toxins-Growth-Rate/toxin-simulations/for_reviewers/"
...
for i in expanded_grid.index.values:
    prod_res_growth_rate = expanded_grid.at[i, 'prod_growth_rate']
    sus_growth_rate = expanded_grid.at[i, 'sus_growth_rate']
    ...
    hours = base_hours / prod_res_growth_rate
    max_cycles = int(hours / dt)
    if (prod_res_growth_rate * sus_growth_rate) <= 0.75:
        max_cycles = 150
    else:
        max_cycles = 75

    producer, resistant, susceptible = make_RPS_cobra_models_biomass_cost(growth_rate = prod_res_growth_rate, 
    ...
    for j in range(0, len(susceptible_locs)):
        ...
        S.reactions.loc[S.reactions.REACTION_NAMES == "EX_carbon_e", "LB"] = -(sus_growth_rate * prod_res_growth_rate)
        ...
        gr_absense_of_toxin = (sus_growth_rate * prod_res_growth_rate)
        toxin_conc_where_effect_starts = 0.
        slope_of_toxin_effect = -(sus_growth_rate * prod_res_growth_rate)
    ...
    biomass_total["prod_res_growth_rate"] = prod_res_growth_rate
    biomass_total["sus_growth_rate"] = sus_growth_rate
    biomass_total["spatial_seed"] = spatial_seed

    metabolites = sim.media
    metabolites["prod_res_growth_rate"] = prod_res_growth_rate
    metabolites["sus_growth_rate"] = sus_growth_rate
    ...
    spatial_data["prod_res_growth_rate"] = prod_res_growth_rate
    spatial_data["sus_growth_rate"] = sus_growth_rate
...
total_biomass_data.to_csv("/Users/abisesi/Desktop/PhD/Projects/Toxins-Growth-Rate/toxin-simulations/for_reviewers/total_biomass_res-prod-same-growth_absolute.csv")
metabolite_data.to_csv("/Users/abisesi/Desktop/PhD/Projects/Toxins-Growth-Rate/toxin-simulations/for_reviewers/metabolites_res-prod-same-growth_absolute.csv")
starting_locations.to_csv("/Users/abisesi/Desktop/PhD/Projects/Toxins-Growth-Rate/toxin-simulations/for_reviewers/starting_locations_res-prod-same-growth_absolute.csv")
