...
# set up treatment parameters, 105 conditions
...
parameter_dict = {'growth_rate': growth_rates, 'production_cost': production_costs, 
                  ...
                  'replicate': replicates, 'space_widths': space_width_starts, 'ratios': ratio}
...
for i in expanded_grid.index.values:
    ...
    base_hours = 1 
    prod_ratio = expanded_grid.at[i, 'ratios']
    ...
    length = int(grid_dim * prod_ratio)
    producer_locs = locs[0:length]
    resistant_locs = locs[length:length+length]
    susceptible_locs = locs[length+length:]
    ...
    biomass_total["prod_ratio"] = prod_ratio
    ...
    metabolites["prod_ratio"] = prod_ratio
    ...
    spatial_data["prod_ratio"] = prod_ratio

    starting_locations = pd.concat([starting_locations, spatial_data])

    print(i + 1, "of", max(expanded_grid.index.values) + 1)

total_biomass_data.to_csv("/Users/abisesi/Desktop/PhD/Projects/Toxins-Growth-Rate/toxin-simulations/for_reviewers/total_biomass_low_ratio.csv")
starting_locations.to_csv("/Users/abisesi/Desktop/PhD/Projects/Toxins-Growth-Rate/toxin-simulations/for_reviewers/starting_locations_low_ratio.csv")
metabolite_data.to_csv("/Users/abisesi/Desktop/PhD/Projects/Toxins-Growth-Rate/toxin-simulations/for_reviewers/metabolites_low_ratio.csv")
